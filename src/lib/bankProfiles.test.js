...
export const BANK_PROFILES = {
  ...
  guayaquil: {
    ...
    buildRecord: (r, ctx) => {
      ...
      const cuentaLimpia = onlyDigits(r.cuenta).replace(/^0+/, '') || '0';
      const cuenta = cuentaLimpia.padStart(8, '0');
...
export function validateRow(bankKey, r) {
  ...
  if (bankKey === 'guayaquil') {
    const cuentaLimpia = onlyDigits(r.cuenta).replace(/^0+/, '');
    if (cuentaLimpia.length > 8) {
      errs.push(`Cuenta tiene ${cuentaLimpia.length} dígitos significativos, Guayaquil solo admite 8`);
    }
  }
